...
fsgd_file_srt_total='/OUTPUTS/srt_total.fsgd'
fsgd_file_srt_total_cons='/OUTPUTS/srt_total_cons.fsgd'
...
#run mri_glm_fit for srt_total
echo "Running mri_glm_fit for left hemisphere for srt_total"
mri_glmfit \
...
#run with clusterwise corrections for negative correlations for srt_total
...
#run mri_glm_fit for srt_total_cons
echo "Running mri_glm_fit for left hemisphere for srt_total_cons"
mri_glmfit \
--y $out_dir/all.lh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_srt_total_cons \
--C $matrix_file \
--surf fsaverage lh \
--cortex \
...
echo "Running mri_glm_fit for right hemisphere for srt_total_cons"
mri_glmfit \
--y $out_dir/all.rh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_srt_total_cons \
--C $matrix_file \
...
--eres-save
#run with clusterwise corrections for negative correlations for srt_total_cons
