#!/bin/bash


in_dir='/INPUTS'
out_dir='/OUTPUTS/DATA'
source='/REPO/src/CHAMP-A018'

mkdir -p $out_dir
mkdir -p $out_dir/glm

fsgd_file_sdmt_corr='/OUTPUTS/sdmt_corr.fsgd'
fsgd_file_sdmt_num='/OUTPUTS/sdmt_num.fsgd'
fsgd_file_srt_recall_delay='/OUTPUTS/srt_recall_delay.fsgd'
matrix_file="${source}/matrix.mtx"

#cp fsaverage to inputs
cp -r /usr/local/freesurfer/subjects/fsaverage /INPUTS/


# for each subject sample mgx vol to left and right hemisphere

for subject in $in_dir/*; do
    echo $subject
    #skip if subject is not a directory
    if [ ! -d $subject ]; then
        continue
    fi

    # derive subject name and skip fsaverage reliably
    subject_name=$(basename "$subject")
    if [ "$subject_name" = "fsaverage" ]; then
        continue
    fi
    
    mkdir -p $out_dir/$subject_name

    sub_feobv=$in_dir/$subject_name/assessors/*/*FEOBVQA_v4*/SUBJ/gtmpvc.esupravwm.output
    sub_surf_w_input=$in_dir/$subject_name/assessors/*/*FEOBVQA_v4*/SUBJ
    
    echo "Before expansion - sub_feobv: $sub_feobv"
    echo "Before expansion - sub_surf_w_input: $sub_surf_w_input"
    
    sub_surf_w_input=$(echo $sub_surf_w_input)

    #select from $sub_surf_w_input the string not including /INPUTS/
    sub_surf=$(echo $sub_surf_w_input | sed "s|$in_dir/||")

    # Expand wildcards to actual paths
    sub_feobv=$(echo $sub_feobv)
    sub_surf=$(echo $sub_surf)
    
    echo "After expansion - sub_feobv: $sub_feobv"
    echo "After expansion - sub_surf: $sub_surf"

    echo "Processing: $sub_feobv"

    mri_vol2surf \
    --mov $sub_feobv/mgx.ctxgm.nii.gz \
    --reg $sub_feobv/aux/bbpet2anat.lta \
    --hemi lh \
    --projfrac 0.5 \
    --o $out_dir/$subject_name/lh.mgx.ctx.fsaverage.sm00.nii.gz \
    --cortex \
    --trgsubject fsaverage \
    --srcsubject $sub_surf \
    --sd $in_dir

    mri_vol2surf \
    --mov $sub_feobv/mgx.ctxgm.nii.gz \
    --reg $sub_feobv/aux/bbpet2anat.lta \
    --hemi rh \
    --projfrac 0.5 \
    --o $out_dir/$subject_name/rh.mgx.ctx.fsaverage.sm00.nii.gz \
    --cortex \
    --trgsubject fsaverage \
    --srcsubject $sub_surf \
    --sd $in_dir


done

#concat all subs into one stack file
echo "Concatenating all left hemispheres"
mri_concat \
--i $out_dir/*/lh.mgx.ctx.fsaverage.sm00.nii.gz \
--o $out_dir/all.lh.mgx.ctx.fsaverage.sm00.nii.gz

echo "Concatenating all right hemispheres"
mri_concat \
--i $out_dir/*/rh.mgx.ctx.fsaverage.sm00.nii.gz \
--o $out_dir/all.rh.mgx.ctx.fsaverage.sm00.nii.gz


#smooth onto surface
echo "Smoothing left hemisphere"
mris_fwhm \
--smooth-only \
--i $out_dir/all.lh.mgx.ctx.fsaverage.sm00.nii.gz \
--o $out_dir/all.lh.mgx.ctx.fsaverage.sm05.nii.gz \
--cortex \
--s fsaverage \
--hemi lh \
--fwhm 5

echo "Smoothing right hemisphere"
mris_fwhm \
--smooth-only \
--i $out_dir/all.rh.mgx.ctx.fsaverage.sm00.nii.gz \
--o $out_dir/all.rh.mgx.ctx.fsaverage.sm05.nii.gz \
--cortex \
--s fsaverage \
--hemi rh \
--fwhm 5

#run mri_glm_fit for smdt_corr
echo "Running mri_glm_fit for left hemisphere for sdmt_corr"
mri_glmfit \
--y $out_dir/all.lh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_sdmt_corr \
--C $matrix_file \
--surf fsaverage lh \
--cortex \
--glmdir $out_dir/glm/lh.glm.glmdir_sdmt_corr \
--eres-save

echo "Running mri_glm_fit for right hemisphere for sdmt_corr"
mri_glmfit \
--y $out_dir/all.rh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_sdmt_corr \
--C $matrix_file \
--surf fsaverage rh \
--cortex \
--glmdir $out_dir/glm/rh.glm.glmdir_sdmt_corr \
--eres-save

#run with clusterwise corrections for negative correlations for sdmt_corr
echo "Running mri_glmfit-sim for left hemisphere for sdmt_corr"
mri_glmfit-sim \
--glmdir $out_dir/glm/lh.glm.glmdir_sdmt_corr \
--perm 1000 4.0 neg \
--cwp 0.05 \
--2spaces \
--bg 1
echo "Running mri_glmfit-sim for right hemisphere for sdmt_corr"
mri_glmfit-sim \
--glmdir $out_dir/glm/rh.glm.glmdir_sdmt_corr \
--perm 1000 4.0 neg \
--cwp 0.05 \
--2spaces \
--bg 1

#run mri_glm_fit for sdmt_num
echo "Running mri_glm_fit for left hemisphere for sdmt_num"
mri_glmfit \
--y $out_dir/all.lh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_sdmt_num \
--C $matrix_file \
--surf fsaverage lh \
--cortex \
--glmdir $out_dir/glm/lh.glm.glmdir_sdmt_num \
--eres-save

echo "Running mri_glm_fit for right hemisphere for sdmt_num"
mri_glmfit \
--y $out_dir/all.rh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_sdmt_num \
--C $matrix_file \
--surf fsaverage rh \
--cortex \
--glmdir $out_dir/glm/rh.glm.glmdir_sdmt_num \
--eres-save
#run with clusterwise corrections for negative correlations for sdmt_num
echo "Running mri_glmfit-sim for left hemisphere for sdmt_num"
mri_glmfit-sim \
--glmdir $out_dir/glm/lh.glm.glmdir_sdmt_num \
--perm 1000 4.0 neg \
--cwp 0.05 \
--2spaces \
--bg 1
echo "Running mri_glmfit-sim for right hemisphere for sdmt_num"
mri_glmfit-sim \
--glmdir $out_dir/glm/rh.glm.glmdir_sdmt_num \
--perm 1000 4.0 neg \
--cwp 0.05 \
--2spaces \
--bg 1

#run mri_glm_fit for srt_recall_delay
echo "Running mri_glm_fit for left hemisphere for srt_recall_delay"
mri_glmfit \
--y $out_dir/all.lh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_srt_recall_delay \
--C $matrix_file \
--surf fsaverage lh \
--cortex \
--glmdir $out_dir/glm/lh.glm.glmdir_srt_recall_delay \
--eres-save
echo "Running mri_glm_fit for right hemisphere for srt_recall_delay"
mri_glmfit \
--y $out_dir/all.rh.mgx.ctx.fsaverage.sm05.nii.gz \
--fsgd $fsgd_file_srt_recall_delay \
--C $matrix_file \
--surf fsaverage rh \
--cortex \
--glmdir $out_dir/glm/rh.glm.glmdir_srt_recall_delay \
--eres-save
#run with clusterwise corrections for negative correlations for srt_recall_delay
echo "Running mri_glmfit-sim for left hemisphere for srt_recall_delay"
mri_glmfit-sim \
--glmdir $out_dir/glm/lh.glm.glmdir_srt_recall_delay \
--perm 1000 4.0 pos \
--cwp 0.05 \
--2spaces \
--bg 1
echo "Running mri_glmfit-sim for right hemisphere for srt_recall_delay"
mri_glmfit-sim \
--glmdir $out_dir/glm/rh.glm.glmdir_srt_recall_delay \
--perm 1000 4.0 pos \
--cwp 0.05 \
--2spaces \
--bg 1
