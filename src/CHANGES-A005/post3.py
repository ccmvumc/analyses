...
def _stemplots(df, pdf):
    ...
    fig.suptitle('SCLIMBIC Basal Forebrain RS Connectivity -- Yeo 7 Networks\n(n=85)')
...
def main(data_dir):
    pdf_file = f'{data_dir}/report3.pdf'
    ...
    df['r2nhr'] = df['r2network'] + '.' + df['r2hemi'] + '.' + df['r2region']
    df = df.sort_values('r2nhr')
    dfx = df[['id', 'r2atlas', 'r2nhr', 'zvalue']]
    dfn = dfx.groupby(['id', 'r2atlas', 'r2nhr']).mean()
    dfn = dfn.reset_index()
