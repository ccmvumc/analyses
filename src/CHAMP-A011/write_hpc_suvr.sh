...
for subject in $in_dir/*; do
    subject_name=$(basename $subject)
    mkdir -p $out_dir/$subject_name
    mri_segstats --seg "${out_dir}/${subject_name}/bilat_hpc.nii.gz" --id 1 --i "${subject}/mri/gtmpvc.output/rbv.nii.gz" --avgwf "${out_dir}/${subject_name}/full_hpc.txt"
    mri_segstats --seg "${subject}/mri/gtmseg.mgz" --id 17 --i "${subject}/mri/gtmpvc.output/rbv.nii.gz" --avgwf "${out_dir}/${subject_name}/Left_hpc.txt"
    mri_segstats --seg "${subject}/mri/gtmseg.mgz" --id 53 --i "${subject}/mri/gtmpvc.output/rbv.nii.gz" --avgwf "${out_dir}/${subject_name}/Right_hpc.txt"
done
