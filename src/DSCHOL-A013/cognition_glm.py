...
second_level_model_stroop = SecondLevelModel(
	mask_img=mask_path, n_jobs=1
)
second_level_model_stroop = second_level_model_stroop.fit(
	FEOBV_img_paths, design_matrix=design_matrix_stroop)
...
z_map_stroop = second_level_model_stroop.compute_contrast(
	second_level_contrast=[1, 0, 0],
	output_type="z_score",
)
...
thresholded_map_stroop, threshold_stroop = threshold_stats_img(z_map_stroop,
												 alpha=threshold_1,
												 cluster_threshold=50)
...
thresholded_map_stroop.to_filename(f'{output_path}/thresholded_stroop_effect_z_map.nii')
...
with PdfPages(pdf_filename) as pdf:
	...
	plotting.plot_stat_map(
		thresholded_map_stroop,
		threshold=threshold_stroop,
		colorbar=True,
		cut_coords=6,
		display_mode="x",
		figure=fig,
