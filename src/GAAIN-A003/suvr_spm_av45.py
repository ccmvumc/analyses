...
if not os.path.exists('/Users/jasonrussell/Documents/OUTPUTS/gaain_A003/av45'):
	os.mkdir('/Users/jasonrussell/Documents/OUTPUTS/gaain_A003/av45')
else:
...
for subject in sorted(os.listdir(out_dir)):
		...
		subject_amyloid = f'{out_dir}/{subject}/av45/mean_pet_av45.nii.gz'
		subject_mr = glob.glob(f'{out_dir}/{subject}/mr/*mr.nii')[0]
	
		print('Amyloid:', subject_amyloid)
	
		subject_out = f'{out_dir}/av45/{subject}'
...
#datasink
datasink = Node(DataSink(base_directory=f'{out_dir}/av45'),
				name = 'datasink')

#make workflow
cl_preproc = Workflow(name='cl_preproc', base_dir = '{out_dir}/av45')
...
for subject in sorted(os.listdir(f'{out_dir}/av45/normalized_final')):
	#apply masks
	pet_mni = f'{out_dir}/av45/normalized_final/{subject}/wrreorient_pet.nii'
...
# Generate pdf report
pdf_filename = f"{out_dir}/av45/report.pdf"
...
with PdfPages(pdf_filename) as pdf:
	for subject in sorted(os.listdir(f'{out_dir}/av45/normalized_final')):
		...
		img = image.load_img(f'{out_dir}/av45/normalized_final/{subject}/wrreorient_pet.nii')
...
output_df.to_csv(f'{out_dir}/av45_centiloid_suvrs.csv', index=False)
